        ...
        if isinstance(handler, dict):
            for part, func in handler.items():
                if first_part is None:
                    EventBindManager.bind_handlers(func, part)
                else:
                    EventBindManager.bind_handlers(func, first_part, part)
            return

        if isinstance(handler, list):
            for func in handler:
                EventBindManager.bind_handlers(func, first_part, second_part)
            return
