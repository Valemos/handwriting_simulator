...
class EntryIntegerWithLabel(EntryWithLabel):
    ...
    def update_integer_field(cls, field):
        if not cls.str_is_int(field.get_path_or_raise()):
            field.set(str(int('0' + ''.join((i for i in field.get_path_or_raise() if i.isdigit())))))

    def get(self):
        self.update_integer_field(self.str_variable)
        return int(self.str_variable.get_path_or_raise())
