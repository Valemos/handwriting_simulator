...
class AnchorEditorWidget(tk.Frame):
    ...
    def enable_edit_anchor_points(self):
        ...
        self.page_manager.start_anchor_editing(self.canvas)
        self.update_anchor_indices()

    def disable_edit_anchor_points(self):
        self.btn_create_lines.config(state=tk.DISABLED)
        self.select_page_switch_handlers()
        self.page_manager.stop_anchor_editing()
        self.update_anchor_indices()
    ...
    def handle_create_more_lines(self, event=None):
        ...
        if self.page_manager.page_exists():
            ...
            if lines_num > 0:
                if not self.page_manager.anchor_manager.add_intermediate_lines(0, 1, lines_num):
    ...
    def constant_redraw_last_anchor(self, point):
        if self.page_manager.is_started_anchor_editing():
            self.page_manager.anchor_manager.redraw_pointer_point(point)
    ...
    def update_anchor_indices(self):
        try:
            indices = self.page_manager.get_anchor_indices()
